	...
	const qtext = {key: {remoteJid: "status@broadcast", participant: "0@s.whatsapp.net"}, message: {"extendedTextMessage": {"text": "[ 𝗚𝗿𝗼𝘂𝗽 𝗡𝗼𝘁𝗶𝗳𝗶𝗰𝗮𝘁𝗶𝗼𝗻 ]"}}}
		if (global.db.groups[id] && global.db.groups[id].welcome == true) {
			...
			for (let n of participants) {
				...
				if (action == 'add') {
	teks = author.split("").length < 1 ? `hai kak @${n.split('@')[0]}\n\n*selamat datang di grup kami*\n*jangan lupa untuk membaca dekripsi ya*\n*terimakasih udah join*` : author !== n ? `*selamat buat ka* @${author.split("@")[0]}\n*kamu telah di tambah*\n*ke grup ini oleh* : @${n.split('@')[0]}` : ``
 let asu = await conn.sendMessage(id, {text: `${teks}`, mentions: [author, n]}, {quoted: qtext})	
}
				 if (action == 'remove') {
	teks = author == n ? `*selamat tinggal ka* : @${n.split('@')[0]}\n*semoga tenang di alam sana*` : author !== n ? `*yah kasian kamu* : @${author.split("@")[0]}\n*di keluarin sama si* : @${n.split('@')[0]} Dari Grup Ini` : ""
	let asu = await conn.sendMessage(id, {text: `${teks}`, mentions: [author, n]}, {quoted: qtext})
}		
				if (action == 'promote') {
	teks = author == n ? `*selamat untuk ka* @${n.split('@')[0]}\n*Telah Menjadi Admin Grup Ini*` : author !== n ? `*yeyy kamu* :  @${n.split('@')[0]}\n*sudah di jadikan admin grup*\n*oleh bang* : @${author.split("@")[0]}` : ""
	let asu = await conn.sendMessage(id, {text: `${teks}`, mentions: [author, n]}, {quoted: qtext})
}
            }
			 if (action == 'demote') {
	teks = author == n ? `*yah kamu* @${n.split('@')[0]} Telah Berhenti Menjadi Admin` : author !== n ? `*turut berdukacita buat ka* : @${n.split('@')[0]}\n*karena sudah bukan admin lagi*\n*di berhentikan olah ka* : @${author.split("@")[0]}\nSebagai Admin Grup Ini` : ""
let asu = await conn.sendMessage(id, {text: `${teks}`, mentions: [author, n]}, {quoted: qtext})
}}
	} catch (e) {
...
async function MessagesUpsert(conn, message, store) {
	try {
		...
		require('../nabzx.js')(conn, m, message, store);
...
async function Serialize(conn, m, store) {
	...
	if (m.message) {
		...
		if (m.quoted) {
			...
			m.quoted.isBaileys = m.quoted.id ? (m.quoted.id.startsWith('3EB0') || m.quoted.id.startsWith('B1E') || m.quoted.id.startsWith('BAE')) : false
